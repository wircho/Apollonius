...
extension Geometry {
  ...
  enum PointParameters<T: Real & Codable> {
    ...
    static func on(_ straight: Straight<T>, cursor: Cursor<T>) -> PointParameters<T> {
      return ._onStraightNormalized(.init(straight), cursor: cursor)
    }
    
    static func on(_ straight: Straight<T>, cursor: Cursor<Length<T>>) -> PointParameters<T> {
      return ._onStraightAbsolute(.init(straight), cursor: cursor)
    }
    
    static func on(_ circular: Circular<T>, cursor: Cursor<SimpleAngle<T>>) -> PointParameters<T> {
      return ._onCircular(.init(circular), cursor: cursor)
    }
    
    static func intersection(_ intersection: Intersection<T>, index: IntersectionIndex) -> PointParameters<T> {
      return ._intersection(.init(intersection), index: index)
    }
    
    static func intersection(_ straight0: Straight<T>, _ straight1: Straight<T>) -> PointParameters<T> {
      return ._twoStraightsIntersection(.init(straight0), .init(straight1))
    }
    
    static func oppositeIntersection(_ intersection: Intersection<T>, from oppositePoint: Point<T>) -> PointParameters<T> {
      return ._oppositeIntersection(.init(intersection), oppositePoint: .init(oppositePoint))
    }
    
    static func circumcenter(_ point0: Point<T>, _ point1: Point<T>, _ point2: Point<T>) -> PointParameters<T> {
